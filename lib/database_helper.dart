import 'package:crud_flutter/libros.dart';
...
class DatabaseHelper {
  ...
  Future<void> insertLibro(Libro item) async{
    ...
    await db.insert('libros', item.toMap(), conflictAlgorithm:
conflictAlgorithm.replace);
  ...
  Future<List<Libro>> getItems() async{
    ...
    return List.generate(maps.length, (i){
        return Libro(id: maps(i)['id'], tituloLibro:
maps [i] ['tituloLibro'] );
    }); 
  }

  Future<int> eliminar(String table, {String? where, List<Object?>
whereArgs }) async{
...
(String? where, List<Object>? whereArgs)) async{
  final db = await _initDatabase();
  return await db.update(table, values, where, where: where, whereArgs:
whereArgs);
}
